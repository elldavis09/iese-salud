...
<template>
  <div class="space-y-8 animate-fade-in-up">
    ...
    <div class="bg-white rounded-3xl p-6 md:p-8 border border-gray-200 shadow-sm relative overflow-hidden">
      <div class="absolute top-0 right-0 w-32 h-32 bg-indigo-50 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"></div>

      <div class="flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
        
        <div class="relative">
          <div class="w-24 h-24 rounded-2xl bg-gradient-to-br from-indigo-100 to-white border border-indigo-200 flex items-center justify-center shadow-inner">
        ...
        <div class="flex-1 text-center md:text-left space-y-2">
          ...
          <div class="flex flex-wrap justify-center md:justify-start gap-3 mt-3">
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold uppercase tracking-wide">
              <i class="ph ph-identification-badge text-lg"></i>
              {{ tutorGrupoHasStudentsStore.student?.rol?.nombre || 'Rol: Estudiante' }}
            </span>
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wide"
    ...
    <div>
      ...
      <div class="grid gap-4">
        ...
        <div v-if="forms?.length === 0" class="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-gray-300">
        ...
        >
          <div class="absolute left-0 top-0 bottom-0 w-1.5 transition-colors duration-300"
               :class="test.status === 'completed' ? 'bg-emerald-500' : 'bg-amber-400'">
          </div>
          ...
          <div class="p-5 pl-7 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-start gap-4">
              <div class="mt-1 p-3 rounded-xl shrink-0 flex items-center justify-center transition-colors"
                   :class="test.status === 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'">
                <i :class="test.status === 'completed' ? 'ph ph-check-circle' : 'ph ph-clock'" class="text-2xl"></i>
              </div>
            ...
            <div class="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-6 sm:pl-0 pt-4 sm:pt-0 border-t sm:border-0 border-gray-100">
              
              <div class="text-right min-w-[100px]">
                <template v-if="test.status === 'completed'">
                  <span class="block text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-0.5">Resultado</span>
                ...
                <template v-else>
                  <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-bold border border-amber-100">
                    Pendiente
                  </span>
                </template>
              </div>

              <div v-if="test.status === 'completed'" 
              ...
              <div v-else class="w-10 h-10"></div> </div>
          </div>
        </div>
